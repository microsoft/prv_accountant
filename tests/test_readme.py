from prv_accountant.privacy_random_variables import (
    PoissonSubsampledGaussianMechanism, GaussianMechanism, LaplaceMechanism
)
from prv_accountant.accountant import PRVAccountant


def test_1():
    prv_a = PoissonSubsampledGaussianMechanism(
        noise_multiplier=0.8, sampling_probability=5e-3
    )
    ...
    eps_low, eps_est, eps_up = accountant.compute_epsilon(
        delta=1e-6, num_self_compositions=[m, n, o]
    )
